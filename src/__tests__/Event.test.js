...
describe('<Event /> Component', () => {
    ...
    test('render event details div', () => {
        EventWrapper.setState({showDetails: true});
        expect(EventWrapper.find('.event-details')).toHaveLength(1); 
    });

    test('render contents of event overview', () => {
        expect(EventWrapper.find('.event-overview').children()).toHaveLength(4);
    });

    test('render contents of event-details', () => {
        EventWrapper.setState({showDetails: true}); 
    ...
    test('expand event details when button is clicked', () => {
        EventWrapper.setState({showDetails: false});
        EventWrapper.find('.details-button').simulate('click'); 
        expect(EventWrapper.state('showDetails')).toBe(true); 
    }); 

    test('collapse event details when button is clicked', () => {
        EventWrapper.setState({showDetails: true}); 
        EventWrapper.find('.details-button').simulate('click');
